...
const Video = (props) => {
  ...
  const [videoName, setVideoName] = useState(videoInfor?.metadata.videoName || '');
    ...
    const handleClickSaveTiltle= async()=>{
      try {
        const data = await axios.put(`http://localhost:8080/video/editVideoName/${thumbId}?videoName=${videoName}`)
        const data2 = await axios.put(`http://localhost:8080/video/editVideoName/${videoId}?videoName=${videoName}`)
  ...
  const handleClickReport = async() =>{
    try {
        ...
        const d1 = await axios.post(`http://localhost:8080/video/uploadReport?videoId=${videoId}&msg=${msg}&userId=${usId}`)
  ...
  useEffect(()=>{
      const runBan = async() =>{
        try {
          const data = await axios.get('http://localhost:8080/video/videoIdLargerThanFive')
          setLists(data.data)
