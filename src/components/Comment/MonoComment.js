...
const MonoComment = ({ comment, fetchComments }) => {
    ...
    const handleUnLike = async () => {
        try {
            await axios.put(`http://localhost:8080/comment/decreaseLikes/${comment.id}?decrement=1`);
            fetchComments(); // Refresh the comments list
        } catch (error) {
            console.error("Error increasing likes:", error);
        }
    ...
    const handleUnDis = async () => {
        try {
            await axios.put(`http://localhost:8080/comment/decreaseDislikes/${comment.id}?decrement=1`);
            fetchComments(); // Refresh the comments list
        } catch (error) {
            console.error("Error increasing dislikes:", error);
        }
    ...
    const handleReplySubmit = async () => {
        if (replyText.trim()) {
            try {
                ...
                await axios.post(`http://localhost:8080/comment/upload`, newReply);
                setReplyText("");
    ...
    useEffect(()=>{
        const handleGetUser = async () =>{
            try {
                const data =await axios.get(`http://localhost:8080/user/listUserbyId/${comment.userId}`)
